...
public class GuardDB implements ClassConstants {
	 ...
	 * @throws IOException
	 */
	public GuardDB(final File inFile) throws IOException {
	...
	/**
	 * Go through database marking certain entities for retention, while maintaining
	 ...
	 * @throws RGSException
	 */
	public void retain(final RgsEnum rgsIter, final PrintWriter log)
			throws IOException, ClassFileException, RGSException {

		// Build database if not already done, or if a mapping has already been
		// generated
		...
		try {
			...
			this.classTree.retainMethod("**", "*", true, null, false, ClassConstants.ACC_NATIVE,
					ClassConstants.ACC_NATIVE);

			// Always retain the auto-generated values() and valueOf(...) methods in Enums,
			// using script entries:
		...
		while (rgsIter.hasNext()) {
			final RgsEntry entry = rgsIter.next();
			try {
				switch (entry.type) {
				case OPTION:
					if (ClassConstants.OPTION_DigestSHA.equals(entry.name)) {
						...
						this.classTree.retainClass("**", true, false, false, false, false, "java/lang/Enum", false, 0,
								0);
						...
						this.classTree.retainClass("**", false, false, false, false, false, "java/applet/Applet", false,
								0, 0);
						...
						// .class **_Stub
						// .class **_Skel
						this.classTree.retainClass("**", false, true, false, false, false, "java/rmi/Remote", false, 0,
								0);
					...
					if (ClassConstants.OPTION_Serializable.equals(entry.name)
						...
						// .method;private **/writeObject (Ljava/io/ObjectOutputStream;)V extends
						// java/io/Serializable
						// .method;private **/readObject (Ljava/io/ObjectInputStream;)V extends
						// java/io/Serializable
						...
						// .field;static;final **/serialPersistentFields [Ljava/io/ObjectStreamField;
						...
						this.classTree.retainMethod("**/readObject", "(Ljava/io/ObjectInputStream;)V", false,
								"java/io/Serializable", false, ClassConstants.ACC_PRIVATE, ClassConstants.ACC_PRIVATE);
						this.classTree.retainMethod("**/writeReplace", "()Ljava/lang/Object;", false,
								"java/io/Serializable", false, 0, 0);
						this.classTree.retainMethod("**/readResolve", "()Ljava/lang/Object;", false,
								"java/io/Serializable", false, 0, 0);
	 ...
	 * @throws IOException
	 */
	public void createMap(final PrintWriter log) throws IOException, ClassFileException {
		// Build database if not already done
		if (this.classTree == null) {
			this.buildClassTree(log);
		}

		// Traverse the class tree, generating obfuscated names within package and class
		// namespaces
		this.classTree.generateNames();

		// Resolve the polymorphic dependencies of each class, generating non-private
		// method and field names for each namespace
		...
		log.println("#");
		log.println(
				GuardDB.LOG_MEMORY_USED + Long.toString(rt.totalMemory() - rt.freeMemory()) + GuardDB.LOG_MEMORY_BYTES);
	 ...
	 * @throws ClassFileException
	 */
	public void remapTo(final File out, final PrintWriter log) throws IOException, ClassFileException {
		...
		// for each class file. Other files are
		// copied through unchanged, except for manifest and any signature files - these
		// are deleted and the manifest is
		// regenerated.
		final Enumeration<? extends ZipEntry> entries = this.inJar.entries();
		...
		try {
			...
			while (entries.hasMoreElements()) {
				...
				try {
					...
					final String inName = inEntry.getName();
					if (inName.length() > GuardDB.CLASS_EXT.length()
						...
						if (cf != null) {
							...
							final Cl cl = this.classTree.getCl(cf.getName());
							// Trim entire class if requested
							if (cl != null) {
								cf.trimAttrs(this.classTree);
								cf.updateRefCount();
								cf.remap(this.classTree, log, this.enableMapClassString);
								final ZipEntry outEntry = new ZipEntry(cf.getName() + GuardDB.CLASS_EXT);
								outJar.putNextEntry(outEntry);

								// Create an OutputStream piped through a number of digest generators for the
								// manifest
							...
							|| inName.length() > GuardDB.SIGNATURE_PREFIX.length() + 1 + GuardDB.SIGNATURE_EXT.length()
									...
									&& inName.substring(inName.length() - GuardDB.SIGNATURE_EXT.length(),
											inName.length()).equals(GuardDB.SIGNATURE_EXT)) {
						...
						if (size != -1) {
							...
							final String outName = this.classTree.getOutName(inName);
			...
			final ZipEntry outEntry = new ZipEntry(GuardDB.STREAM_NAME_MANIFEST);
			outJar.putNextEntry(outEntry);
			final PrintWriter writer = new PrintWriter(new BufferedWriter(new OutputStreamWriter(outJar)));
	...
	private void parseManifest() throws IOException {
		// The manifest file is the first in the jar and is called (case insensitively)
		// 'MANIFEST.MF'
		...
		for (final Section section : this.oldManifest) {
			if (section != null && section != version) {
				final Header name = section.findTag(GuardDB.MANIFEST_NAME_TAG);
				if (name == null) {
	 ...
	 * @param digests
	 */
	private void updateManifest(final String inName, final String outName, final List<MessageDigest> digests) {
		// Check for section in old manifest
		final Section oldSection = this.oldManifest.find(GuardDB.MANIFEST_NAME_TAG, inName);
		if (oldSection != null) {
			...
			for (final Header header : oldSection) {
