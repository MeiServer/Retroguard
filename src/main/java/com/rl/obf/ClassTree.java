...
/**
 * Tree structure of package levels, classes, methods and fields used for
 * obfuscation.
 * 
 * @author Mark Welsh
 */
public class ClassTree implements NameMapper, ClassConstants {
	...
	/**
	 * Update the path of the passed filename, if that path corresponds to a
	 * package.
	 * 
	 * @param inName
	 */
	...
	/**
	 * Mark a class/interface type (and possibly methods and fields defined in
	 * class) for retention.
	 * 
	 * @param name
	...
	public void retainClass(String name, boolean retainToPublic, boolean retainToProtected, boolean retainPubProtOnly,
			boolean retainFieldsOnly, boolean retainMethodsOnly, String extendsName, boolean invert, int accessMask,
			int accessSetting) throws ClassFileException {
		// Mark the class (or classes, if this is a wildcarded specifier)
	...
	/**
	 * Get class in tree from the fully qualified name, returning null if name not
	 * found.
	 * 
	 * @param fullName
	 * @throws ClassFileException
	...
	public Cl getCl(String fullName) throws ClassFileException {
		...
		if (!(ti instanceof Cl)) {
			// 15Jul2005 - this exception is being over-sensitive with fullName of null
			// (should never get here) so safely return
			// null instead
			// throw new ClassFileException("Inconsistent class or interface name: " +
			// fullName);
			return null;
		}
		return (Cl) ti;
	...
	/**
	 * Get package in tree from the fully qualified name, returning null if name not
	 * found.
	 * 
	 * @param fullName
	 * @throws ClassFileException
	...
	public String mapDescriptor(String descriptor) throws ClassFileException {
		// Pass everything through unchanged, except for the String between 'L' and ';'
		// -- this is passed through mapClass(String)
				...
				public void packageAction(Pk pk) {
					if (!pk.isFromScript()) {
						if (pk.getRepackageName() != null) {
							log.println(RgsEntryType.REPACKAGE_MAP + " " + pk.getFullInName(true) + " "
									+ pk.getRepackageName(true));
						} else {
							log.println(RgsEntryType.PACKAGE_MAP + " " + pk.getFullInName(true) + " "
									+ pk.getOutName(true));
						}
					}
				}
	...
	/**
	 * Walk the whole tree taking action once only on each package level, class,
	 * method and field.
	 * 
	 * @param ta
	 * @throws ClassFileException
