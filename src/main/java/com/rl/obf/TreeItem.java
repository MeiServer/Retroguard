...
public class TreeItem {
	...
	public static boolean isMatch(String pattern, String string) {
		...
		if (pattern.charAt(0) != '*') {
			pos = pattern.indexOf('*');
			final String head = pattern.substring(0, pos);
			if ((string.length() < head.length()) || !string.substring(0, head.length()).equals(head)) {
				return false;
			}
		...
		if (pattern.charAt(pattern.length() - 1) != '*') {
			...
			final String tail = pattern.substring(pos + 1);
			if ((string.length() < tail.length()) || !string.substring(string.length() - tail.length()).equals(tail)) {
				return false;
			}
		...
		for (final String chunk : section) {
	...
	/**
	 * Do a generalized wildcard String match - handles '**' wildcard that matches
	 * across package boundaries.
	 * 
	 * @param pattern
	 * @param string
	 */
	public static boolean isGMatch(final String pattern, final String string) {
		PatternList pl, sl;
	 ...
	 * @param name
	 */
	public TreeItem(final TreeItem parent, final String name) {
		this.parent = parent;
		this.inName = name;
		if (parent != null) {
	 ...
	 * @param setting
	 */
	public boolean modifiersMatchMask(final int mask, final int setting) {
	 ...
	 * @param outName
	 */
	public void setOutName(final String outName) {
	 ...
	 * Return the output name of the entry, obfuscated or original.
	 */
	public String getOutName(final boolean output) {
	 ...
	 * @param parent
	 */
	public void setParent(final TreeItem parent) {
		this.parent = parent;
	}
	 ...
	 */
	public String getFullInName(final boolean output) {
	...
	/**
	 * Construct and return the full modified name of the entry, optionally using .
	 * for default package
	 * 
	 * @param output
	 *            replace default package with .
	 */
	public String getFullOutName(final boolean output) {
	 ...
	 */
	public static String joinName(final String parentName, final String sep, final String name) {
	 ...
	 * @param pattern
	 */
	public boolean isOldStyleMatch(final String pattern) {
	 ...
	 * @param pattern
	 */
	public boolean isWildcardMatch(final String pattern) {
