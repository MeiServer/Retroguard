...
public class StackMapFrameInfo {
	 ...
	 */
	public static StackMapFrameInfo create(final DataInput din) throws IOException, ClassFileException {
		final StackMapFrameInfo smfi = new StackMapFrameInfo();
		smfi.read(din);
		return smfi;
	}
	...
	private void read(final DataInput din) throws IOException, ClassFileException {
		...
		if (StackMapFrameInfo.SAME_MIN <= this.u1frameType && this.u1frameType <= StackMapFrameInfo.SAME_MAX) {
			...
			this.u2offsetDelta = din.readUnsignedShort();
		} else if (StackMapFrameInfo.APPEND_MIN <= this.u1frameType
				&& this.u1frameType <= StackMapFrameInfo.APPEND_MAX) {
			this.u2offsetDelta = din.readUnsignedShort();
			final int u2numberOfLocals = 1 + this.u1frameType - StackMapFrameInfo.APPEND_MIN;
			this.readLocals(din, u2numberOfLocals);
		} else if (this.u1frameType == StackMapFrameInfo.FULL_FRAME) {
			this.u2offsetDelta = din.readUnsignedShort();
			final int u2numberOfLocals = din.readUnsignedShort();
			this.readLocals(din, u2numberOfLocals);
			final int u2numberOfStackItems = din.readUnsignedShort();
	...
	protected void markUtf8Refs(final ConstantPool pool) throws ClassFileException {
		for (final VerificationTypeInfo vt : this.stack) {
			vt.markUtf8Refs(pool);
		}
		for (final VerificationTypeInfo vt : this.locals) {
			vt.markUtf8Refs(pool);
		}
	}
	 ...
	 * @throws ClassFileException
	 */
	private void readLocals(final DataInput din, final int items) throws IOException, ClassFileException {
		this.locals = new ArrayList<>(items);
	 ...
	 * @throws ClassFileException
	 */
	private void writeLocals(final DataOutput dout) throws IOException, ClassFileException {
		for (final VerificationTypeInfo vt : this.locals) {
			vt.write(dout);
		}
	}
	 ...
	 * @throws ClassFileException
	 */
	private void readStackItems(final DataInput din, final int items) throws IOException, ClassFileException {
	 ...
	 * @throws ClassFileException
	 */
	private void writeStackItems(final DataOutput dout) throws IOException, ClassFileException {
		for (final VerificationTypeInfo vt : this.stack) {
			vt.write(dout);
		}
	}
