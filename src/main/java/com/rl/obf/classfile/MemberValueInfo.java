...
public class MemberValueInfo {
	 ...
	 */
	public static MemberValueInfo create(final DataInput din) throws IOException, ClassFileException {
		final MemberValueInfo mvi = new MemberValueInfo();
		mvi.read(din);
		return mvi;
	}
	...
	protected void markUtf8Refs(final ConstantPool pool) throws ClassFileException {
		...
		case '@':
			...
			break;
		case '[':
			for (final MemberValueInfo mv : this.values) {
				mv.markUtf8Refs(pool);
			}
			break;
		default:
			throw new ClassFileException(
					"Illegal tag value in annotation attribute member_value structure: " + this.u1tag);
		}
	}
	...
	private void read(final DataInput din) throws IOException, ClassFileException {
		...
		case '[':
			...
			this.values = new ArrayList<>(u2numValues);
	...
	protected void remap(final ClassFile cf, final NameMapper nm) throws ClassFileException {
		...
		case 'e':
			break;
		case 'c':
			final String oldDesc = cf.getUtf8(this.u2classInfoIndex);
