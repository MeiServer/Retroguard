...
public class InnerClassesInfo {
	 ...
	 * @throws IOException
	 */
	public static InnerClassesInfo create(final DataInput din) throws IOException {
		final InnerClassesInfo ici = new InnerClassesInfo();
		ici.read(din);
		return ici;
	}
	 ...
	 * @param index
	 */
	protected void setInnerNameIndex(final int index) {
	...
	protected void remap(final ClassFile cf, final NameMapper nm) throws ClassFileException {
		if (this.u2innerNameIndex != 0) {
			...
			final ClassCpInfo innerClassInfo = (ClassCpInfo) cf.getCpEntry(this.u2innerClassInfoIndex);
			final String innerClassName = cf.getUtf8(innerClassInfo.getNameIndex());
