...
public class NestMembersAttrInfo extends AttrInfo {
	 ...
	 * @param attrLength
	 */
	protected NestMembersAttrInfo(final ClassFile cf, final int attrNameIndex, final int attrLength) {
	...
	protected void readInfo(final DataInput din) throws IOException, ClassFileException {
		final int count = din.readUnsignedShort();
		this.classes = new ArrayList<>(count);
	...
	public void writeInfo(final DataOutput dout) throws IOException, ClassFileException {
		...
		for (final int cls : this.classes) {
			dout.writeShort(cls);
		}
	}
}