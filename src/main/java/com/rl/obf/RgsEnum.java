...
public class RgsEnum {
	...
	public static final String[] CLASS_ACCESS = { RgsEnum.ACCESS_PUBLIC, RgsEnum.ACCESS_FINAL, RgsEnum.ACCESS_INTERFACE,
			RgsEnum.ACCESS_ABSTRACT, RgsEnum.ACCESS_ANNOTATION, RgsEnum.ACCESS_ENUM };

	public static final String[] METHOD_ACCESS = { RgsEnum.ACCESS_PUBLIC, RgsEnum.ACCESS_PRIVATE,
			RgsEnum.ACCESS_PROTECTED, RgsEnum.ACCESS_STATIC, RgsEnum.ACCESS_FINAL, RgsEnum.ACCESS_SYNCHRONIZED,
			RgsEnum.ACCESS_BRIDGE, RgsEnum.ACCESS_VARARGS, RgsEnum.ACCESS_NATIVE, RgsEnum.ACCESS_ABSTRACT,
			RgsEnum.ACCESS_STRICT };

	public static final String[] FIELD_ACCESS = { RgsEnum.ACCESS_PUBLIC, RgsEnum.ACCESS_PRIVATE,
			RgsEnum.ACCESS_PROTECTED, RgsEnum.ACCESS_STATIC, RgsEnum.ACCESS_FINAL, RgsEnum.ACCESS_VOLATILE,
			RgsEnum.ACCESS_TRANSIENT, RgsEnum.ACCESS_ENUM };
	...
	// Fields ----------------------------------------------------------------
	private final StreamTokenizer tk;
	 ...
	 * @throws RGSException
	 */
	private static int toAccessFlag(final String accessString) throws RGSException {
	...
	/**
	 * Decode a list of access flags into a bit mask for class, method, or field
	 * access flag u2's.
	 * 
	 * @param entryType
	 * @param accessString
	 * @throws RGSException
	 */
	private static int decodeAccessFlags(final RgsEntryType entryType, String accessString) throws RGSException {
		int accessMask = 0;
		int accessSetting = 0;
		while (accessString != null && accessString.length() >= 5) // ';enum'
		{
			...
			final int endIndex = accessString.indexOf(';', startIndex);
			final String flagString = endIndex == -1 ? accessString.substring(startIndex)
			...
			final int flag = RgsEnum.toAccessFlag(flagString);
			accessMask |= flag;
			if (!invert) {
				accessSetting |= flag;
			}
	 ...
	 * @param rgs
	 */
	public RgsEnum(final InputStream rgs) {
		this.tk = new StreamTokenizer(new BufferedReader(
				rgs != null ? (Reader) new InputStreamReader(rgs) : (Reader) new StringReader(RgsEnum.DEFAULT_RGS)));
		this.tk.resetSyntax();
	...
	public RgsEntry next() throws RGSException {
		final RgsEntry thisOne = this.next;
		final RGSException thisException = this.nextException;
		this.readNext();
				...
				if (ttype == StreamTokenizer.TT_WORD) {
					if (directive == null) {
						...
						case ATTR:
							...
							break;
						case NOWARN:
							// this.checkClassWCSpec(this.tk.sval); // TODO this check doesn't handle
							// wildcards properly
							entry = new RgsEntry(directive, this.tk.sval);
							break;
						case CLASS:
						...
						case NOT_METHOD:
							if (name == null) {
								name = this.tk.sval;
								// this.checkMethodOrFieldSpec(name); // TODO this check doesn't handle
								// wildcards properly
							} else if (descriptor == null) {
								descriptor = this.tk.sval;
								// this.checkMethodDescriptor(descriptor); // TODO this check doesn't handle
								// wildcards properly
						...
						case NOT_FIELD:
							if (name == null) {
								...
								descriptor = this.tk.sval;
								// this.checkJavaType(descriptor); // TODO this check doesn't handle wildcards
								// properly
								entry = new RgsEntry(directive, name, descriptor);
								entry.accessMask = accessMask;
								entry.accessSetting = accessSetting;
						...
						case REPACKAGE_MAP:
							if (name == null) {
								...
								final String obfName = this.tk.sval;
	 ...
	 * @throws RGSException
	 */
	private static void checkJavaType(final String s) throws RGSException {
	 ...
	 * @throws RGSException
	 */
	private static void checkMethodOrFieldSpec(final String s) throws RGSException {
		if (s.length() == 0) {
	 ...
	 * @throws RGSException
	 */
	private static void checkJavaIdentifier(final String s) throws RGSException {
	 ...
	 * @throws RGSException
	 */
	private static void checkJavaInnerIdentifier(final String s) throws RGSException {
		if (s.length() == 0) {
